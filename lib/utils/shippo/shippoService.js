...
class ShippoService {
  ...
  // Create store address (fallback to default config)
  getStoreAddress() {
    return {
  ...
  // Format store owner's shipping address for Shippo
  formatStoreOwnerAddress(storeShippingAddress) {
    if (!storeShippingAddress) {
      throw new Error("Store shipping address is required");
    }

    return {
      name: storeShippingAddress.name || "Store Owner",
      street1: storeShippingAddress.line1,
      street2: storeShippingAddress.line2 || "",
      city: storeShippingAddress.city,
      state: storeShippingAddress.state,
      zip: storeShippingAddress.postal_code,
      country: storeShippingAddress.country,
      phone: "555-123-4567", // Default phone - could be added to user model later
      email: "store@example.com", // Default email - could be added to user model later
    };
  }
  ...
  async createShipment(order, storeOwnerShippingAddress = null) {
    try {
      if (!order.shippingAddress) {
        throw new Error("Order must have a shipping address");
      }

      // Use store owner's address if provided, otherwise fall back to default
      const fromAddress = storeOwnerShippingAddress
        ? this.formatStoreOwnerAddress(storeOwnerShippingAddress)
