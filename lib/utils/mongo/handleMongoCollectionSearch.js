...
export default async function handleMongoCollectionSearch({
  ...
  for (const [field, value] of Object.entries(specificSearchParams)) {
    ...
    // ! Skip parameters that are handled by PostsColSpecialHandling
    if (
      [
        "notificationType",
        "feedType",
    ...
    // ! Special handling for search query (q) parameter for directlinks and landingpages
    if (
      field === "q" &&
      value &&
      ["directlinks", "landingpages"].includes(collectionName)
    ) {
      // Search by name or username using $or operator with case-insensitive regex
      query.$or = [
        { name: { $regex: value.toString(), $options: "i" } },
        { username: { $regex: value.toString(), $options: "i" } },
      ];
      continue;
    }
    ...
    // ! Special handling for fileUrl_contains parameter for GIF filtering
    if (field === "fileUrl_contains" && value) {
      query.fileUrl = { $regex: value, $options: "i" };
      continue;
    }

    // ! Special handling for fileUrl_not_contains parameter to exclude file types (like GIFs)
    if (field === "fileUrl_not_contains" && value) {
      query.fileUrl = query.fileUrl || {};
      query.fileUrl.$not = { $regex: value, $options: "i" };
      continue;
    }
