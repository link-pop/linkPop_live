...
export async function checkFreePlanFeaturesLimit({
  ...
  try {
    ...
    const isDirectLinkOrLandingPage = ["directlinks", "landingpages"].includes(
      entityType
    );
    if (isDirectLinkOrLandingPage && mongoUser?._id) {
      ...
      if (allUserEntities.length <= 1) {
        ...
        if (
          allUserEntities.length === 0 ||
          (earliestCreatedAt &&
            new Date().getTime() - earliestCreatedAt.getTime() <
              (isDevMode ? 1 : 7 * 24 * 60 * 60 * 1000))
        ) {
          return {
            hasAccess: true,
            isFreeTrialPeriod: true,
            timeRemainingMs: earliestCreatedAt
              ? (isDevMode ? 1 : 7 * 24 * 60 * 60 * 1000) -
                (new Date().getTime() - earliestCreatedAt.getTime())
              : isDevMode
              ? 1
