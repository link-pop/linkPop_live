...
/**
 * Update user's lastUploadedCreatorTags with new creator tags
 * @param {string} userId - User ID
 * @param {Object} creatorTags - Object with raceEthnicity, hairColor, bodyType arrays
 * @returns {Promise<Object>} Result of the operation
 */
export const updateCreatorTags = async ({ userId, creatorTags }) => {
  ...
  try {
    ...
    // Initialize lastUploadedCreatorTags if it doesn't exist
    const currentTags = user.lastUploadedCreatorTags || {
      raceEthnicity: [],
      hairColor: [],
      bodyType: [],
    };
    ...
    const updatedUser = await Model.findByIdAndUpdate(
      userId,
      { lastUploadedCreatorTags: updatedTags },
      { new: true }
    ).lean();

    if (!updatedUser) {
      return { error: "Failed to update user creator tags" };
    }

    return { success: true, lastUploadedCreatorTags: updatedTags };
  } catch (err) {
    console.error("Error updating creator tags:", err);
    await sendErrorToAdmin({
      error: err,
      subject: "Update Creator Tags Error",
      context: { userId, creatorTags },
    });
    return handleError(err, "updateCreatorTags", { userId, creatorTags });
  }
};
