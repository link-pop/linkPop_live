...
/**
 * Apply price filter to creators list
 ...
 * @returns {Array} Filtered creators array
 */
const applyPriceFilter = (creators, showPaidOnly) => {
  return creators.filter((creator) => {
    ...
    if (showPaidOnly) {
      ...
      // Show only free creators (price = 0)
      return price === 0;
    }
  });
};
...
export const getSuggestedUsers = async (
  ...
  try {
    ...
    if (mongoUser?.profileType === "fan") {
      ...
      console.log("  hairColor:", mongoUser.hairColor);
      console.log("  bodyType:", mongoUser.bodyType);
      ...
      if (userHasPreferences) {
        ...
        // Build base query for creators with lastUploadedCreatorTags
        const preferenceFilters = {
          ...filters, // Include base filters
        ...
        // Get ALL creators first, then filter by preferences using lastUploadedCreatorTags
        ...
        console.log(
          "📊 Found",
          allCreators.length,
          "total creators for traditional matching"
        );
        ...
        for (const creator of creatorsWithTags) {
          ...
          if (
            ...
            if (
              ...
              mongoUser.hairColor !== creatorMostFrequentTags.hairColor
            ) {
              isMatch = false;
              console.log(
                "  ❌ Hair color mismatch:",
              ...
              console.log("  ✅ Hair color match:", mongoUser.hairColor);
            }
          } else {
            console.log(
          ...
          if (
            ...
            if (
              ...
              mongoUser.bodyType !== creatorMostFrequentTags.bodyType
            ) {
              isMatch = false;
              console.log(
                "  ❌ Body type mismatch:",
              ...
              console.log("  ✅ Body type match:", mongoUser.bodyType);
            }
          } else {
            console.log(
        ...
        if (matchingCreators.length > 0) {
          ...
          // Apply price filter
          const filteredCreators = applyPriceFilter(
            matchingCreators,
            showPaidOnly
          );
          console.log(
            ...
            "creators (showPaidOnly:",
            showPaidOnly,
            ")"
          );

          // Limit results
          const limitedResults = filteredCreators.slice(0, limit);
      ...
      if (
        ...
        if (hasVisitedCreatorsTags) {
          ...
          if (
            ...
            // Get ALL creators first, then filter by comparing most frequent tags
            const allCreators = await getAll({
              col: "users",
              data: {
                ...filters,
      ...
      // Apply price filter
      const filteredFallbackUsers = applyPriceFilter(
        fallbackUsers,
        showPaidOnly
      );
      console.log(
        "💰 After price filter:",
        filteredFallbackUsers.length,
        "creators (showPaidOnly:",
        showPaidOnly,
        ")"
      );
      ...
      // Limit results
      const limitedFallbackUsers = filteredFallbackUsers.slice(0, limit);
    ...
    // Apply price filter
    const filteredUsers = applyPriceFilter(usersWithTags, showPaidOnly);
    console.log(
      "💰 Non-fan users after price filter:",
