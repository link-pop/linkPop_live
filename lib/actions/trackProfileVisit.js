...
export const trackProfileVisit = async (visitedUserId) => {
  ...
  try {
    ...
    // Only track visits to creators
    if (visitedUser.profileType !== "creator") {
      return { success: true, message: "Not a creator profile" };
    }
    ...
    const creatorTagsToAdd = {
      ...
      gender: mostFrequentTags.gender ? [mostFrequentTags.gender] : [],
      age: visitedUser.age ? [visitedUser.age] : [], // Age comes from user profile, not uploaded tags
    };
    ...
    const hasAnyTags =
      ...
      creatorTagsToAdd.gender.length > 0 ||
      creatorTagsToAdd.age.length > 0;
