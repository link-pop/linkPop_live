...
export const createShippingLabel = async ({ orderId, rateId }) => {
  try {
    ...
    try {
      console.log(
        `[createShippingLabel] Sending shipping notification for order ${order.orderNumber}`
      );
      ...
      if (notificationResult.error) {
        console.error(
          `[createShippingLabel] Failed to send shipping notification:`,
          notificationResult.error
        );
        // Don't fail the label creation if notification fails
      } else {
        console.log(
          `[createShippingLabel] ✅ Shipping notification sent for order ${order.orderNumber}`
        );
      }
    } catch (notificationError) {
      console.error(
        `[createShippingLabel] Error sending shipping notification:`,
        notificationError
      );
      // Don't fail the label creation if notification fails
    }
