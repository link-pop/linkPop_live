...
import UserModel from "../../chatServer/models/UserModel";
...
export async function checkUserOwnership(document) {
  ...
  try {
    ...
    // Special handling for storeitemsorders - both buyer and store owner can modify
    if (
      document.storeOwner &&
      ...
      console.log("Detected storeitemsorders document");

      // Handle both ObjectId and string formats for storeOwner and createdBy
      const storeOwnerId = document.storeOwner?._id
        ? document.storeOwner._id.toString()
        : document.storeOwner?.toString();
      const createdById = document.createdBy?._id
      ...
      const isOwner = isStoreOwner || isBuyer;

      console.log("Store items order ownership check:", {
        currentUserId,
        storeOwnerId,
        createdById,
        isStoreOwner,
