...
export const sendErrorToAdmin = async ({
  ...
  // ! Only send error if not in NEXT_PUBLIC_DEV_MODE
  if (
    process.env.NEXT_PUBLIC_DEV_MODE === "true" ||
    process.env.NEXT_PUBLIC_DEV_MODE === true
  ) {
    // In dev mode, skip sending admin email but log for debugging
    console.log("📧 [NEXT_PUBLIC_DEV_MODE] Would send admin email:", {
      subject,
      error: error?.message,
      context,
    });
    return { skipped: true, reason: "NEXT_PUBLIC_DEV_MODE enabled" };
  }
  try {
    if (!error) {
    ...
    const errorDetails = {
      ...
      environment: process.env.NEXT_PUBLIC_DEV_MODE
        ? "development (NEXT_PUBLIC_DEV_MODE)"
        : "production",
    };
