...
export async function addAttachmentsToUserList(listId, attachmentIds) {
  try {
    ...
    // Get the last 3 attachment URLs for preview
    const lastAttachmentUrls = await getLastAttachmentUrls(newAttachmentIds);

    // Update the list with new attachment IDs and lastAttachmentUrls
    const result = await update({
      col: "userlists",
      data: { _id: listId },
      update: {
        attachmentIds: newAttachmentIds,
        lastAttachmentUrls: lastAttachmentUrls,
        updatedAt: new Date(),
      },
