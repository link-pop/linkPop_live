...
import { getSocialLinksCollectionName } from "../utils/linkProtection";
import {
  createProtectedSocialLink,
  updateProtectedSocialLink,
} from "./createProtectedSocialLink";
...
export async function addOrUpdateSocialMediaLink(formData) {
  ...
  try {
    ...
    if (linkId) {
      ...
      result = await updateProtectedSocialLink(linkId, updateData, colName);
      ...
      result = await createProtectedSocialLink(newLinkData, colName);
