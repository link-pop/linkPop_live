...
export async function searchUserLists(
  ...
  try {
    ...
    const userLists = await UserList.find({
      ...
      targetCollection: { $in: targetCollections },
      name: { $regex: searchRegex },
      active: true, // Only active lists
    })
      .select(
        "_id name description slug targetCollection color icon sortOrder lastAttachmentUrls attachmentIds userIds lastUserProfileImages"
      )
      .sort({ sortOrder: 1, name: 1 })
      .lean();

    // Add attachment/user count to each list based on target collection
    const listsWithCounts = userLists.map((list) => ({
      ...
      lastUserProfileImages: list.lastUserProfileImages || [],
    }));
