...
export const getAllPostsOwner = async (args) => {
  ...
  const dataWithFilters =
    col.name === "chatrooms"
      ...
      ? {
          $and: [...(data.$and || [data]), typeFilter, postLabelFilter],
        }
      : { ...data, ...postLabelFilter };

  // Fetch liked posts, bookmarked posts, hidden posts, hidden users, user purchases, and all purchases for owned messages
  const [
    posts,
    likedPostIds,
    bookmarkedPostIds,
