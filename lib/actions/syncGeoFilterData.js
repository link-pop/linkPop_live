 ...
 * @param {string} revalidatePath - Path to revalidate after sync (optional)
 * @returns {Promise<boolean>} - Whether the sync was successful
 */
export const syncGeoFilterData = async (
  entityId,
  entityType,
  revalidatePath = null
) => {
  ...
  try {
    ...
    // Update the entity using the crud update function to ensure proper revalidation
    ...
    const result = await update({
      ...
      skipOwnershipCheck: true, // Skip ownership check for system operations
    });
