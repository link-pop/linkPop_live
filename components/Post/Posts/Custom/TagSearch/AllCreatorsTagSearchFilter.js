...
import { useAllCreatorsPopularTags } from "@/hooks/useAllCreatorsPopularTags";
import SearchTag from "@/components/ui/shared/SearchTag/SearchTag";

export default function AllCreatorsTagSearchFilter({ onTagsChange = null }) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const [searchInput, setSearchInput] = useState("");
  const [selectedTags, setSelectedTags] = useState([]);

  // Get popular tags from all creators' attachments
  const {
    tags: allCreatorsPopularTags,
    isLoading: isLoadingTags,
    error: tagsError,
  } = useAllCreatorsPopularTags(50);
  ...
  const filteredSuggestions = useMemo(() => {
    if (!searchInput.trim() || !allCreatorsPopularTags) return [];

    const input = searchInput.toLowerCase();
    return allCreatorsPopularTags
      .filter((tag) => tag.includes(input) && !selectedTags.includes(tag))
      .slice(0, 10);
  }, [searchInput, selectedTags, allCreatorsPopularTags]);
  ...
  const updateURL = (tags) => {
    ...
    console.log("AllCreatorsTagSearchFilter - Updating URL to:", newURL);
    console.log("AllCreatorsTagSearchFilter - Selected tags:", tags);
  ...
  // Don't render anything if no tags are available (moved after all hooks)
  if (!allCreatorsPopularTags || allCreatorsPopularTags.length === 0) {
    return null;
  }
  ...
  return (
    <div className="w-full mb-4 p-4 bg-background border border-border rounded-lg">
      ...
      {/* Popular Tags from All Creators */}
      <div>
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm font-medium text-foreground">
            Popular Tags
        ...
        <div className="flex flex-wrap gap-2">
          {allCreatorsPopularTags.slice(0, 15).map((tag) => (
            <button
              key={tag}
              onClick={() => addTag(tag)}
