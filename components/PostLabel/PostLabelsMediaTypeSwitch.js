...
export default function PostLabelsMediaTypeSwitch({
  mongoUser,
  allPostLabels = [],
  customListFeedPosts = [],
  ...
  const {
    ...
    queryKey: [
      "postLabelsMediaCounts",
      mongoUser?._id,
      currentListContext?.id || "default",
      allPostLabels.length,
    ...
    queryFn: async () => {
      ...
      try {
        ...
        console.error("❌ Error calculating post label media counts:", error);
        return mediaTypes.reduce((acc, type) => {
          acc[type.value] = 0;
          return acc;
        }, {});
