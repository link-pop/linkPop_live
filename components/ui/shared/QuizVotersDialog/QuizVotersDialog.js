...
import { getQuizVoters } from "@/lib/actions/getQuizVoters";

export default function QuizVotersDialog({ quiz, optionIndex, onClose }) {
  ...
  useEffect(() => {
    loadVoters();
  }, [quiz?._id, optionIndex]);

  const loadVoters = async () => {
    if (!quiz?._id || optionIndex === undefined) return;

    setIsLoading(true);
    try {
      const result = await getQuizVoters(quiz._id, optionIndex);
      ...
      console.error("❌ Error loading quiz voters:", error);
      toastSet({
        isOpen: true,
        title: t("error"),
